...
const AddTrip = () => {
  ...
  const [form, setForm] = useState({ bus_id: '', area: '', departure_time: '', direction: 'from_university' })
  ...
  useEffect(() => { fetchBuses(); loadEditData() }, [])
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: isDark ? '#0f1117' : '#f5f5f5' }}>
      ...
      <div className="max-w-lg mx-auto py-6 px-4">
        ...
        {message && (
          <div className={`mb-4 p-3 rounded-lg text-sm text-center ${message.includes('successfully') || message.includes('تم') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            {message}
          </div>
        )}

        <form onSubmit={handleSubmit} className="rounded-xl shadow-sm p-5 border-t-4"
          style={{ backgroundColor: isDark ? '#1a1d27' : 'white', borderTopColor: '#1e2d5a' }}>

          <div className="mb-4">
            <label className="block text-sm font-medium mb-1" style={{ color: isDark ? '#ffffff' : '#1e2d5a' }}>{t('area')}</label>
            <input type="text" name="area" value={form.area} onChange={handleChange} placeholder="e.g. طبربور" required
              className="w-full border-2 rounded-lg px-3 py-2 text-sm focus:outline-none transition" style={inputStyle}
              onFocus={(e) => e.target.style.borderColor = '#1e2d5a'} onBlur={(e) => e.target.style.borderColor = isDark ? '#2d3748' : '#e5e7eb'} />
          </div>

          <div className="mb-4">
            <label className="block text-sm font-medium mb-1" style={{ color: isDark ? '#ffffff' : '#1e2d5a' }}>{t('bus')}</label>
            <select name="bus_id" value={form.bus_id} onChange={handleChange} required
              className="w-full border-2 rounded-lg px-3 py-2 text-sm focus:outline-none transition" style={inputStyle}
              onFocus={(e) => e.target.style.borderColor = '#1e2d5a'} onBlur={(e) => e.target.style.borderColor = isDark ? '#2d3748' : '#e5e7eb'}>
              <option value="">{t('bus')}...</option>
              {buses.map(bus => <option key={bus.id} value={bus.id}>{bus.bus_number} — {bus.capacity} seats</option>)}
            </select>
          </div>

          <div className="mb-4">
            <label className="block text-sm font-medium mb-1" style={{ color: isDark ? '#ffffff' : '#1e2d5a' }}>{t('departureTime')}</label>
          ...
          <div className="mb-5">
            <label className="block text-sm font-medium mb-1" style={{ color: isDark ? '#ffffff' : '#1e2d5a' }}>{t('direction')}</label>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading} className="flex-1 text-white py-2 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50 transition btn-press" style={{ backgroundColor: '#8B1A2B' }}>
              {loading ? t('saving') : editId ? t('saveChanges') : t('saveTrip')}
            </button>
            <button type="button" onClick={() => navigate('/dashboard')} className="flex-1 border-2 py-2 rounded-lg text-sm hover:opacity-80 transition btn-press" style={{ borderColor: '#1e2d5a', color: isDark ? '#ffffff' : '#1e2d5a', backgroundColor: 'transparent' }}>
              {t('cancel')}
            </button>
          </div>
