...
const Dashboard = () => {
  ...
  const handleRestore = async (tripId) => {
    try {
      ...
      setToast({ message: 'Failed to restore trip', type: 'error' })
    }
  }
    ...
    <div className="min-h-screen" style={{ backgroundColor: isDark ? '#0f1117' : '#f5f5f5' }}>
      ...
      <div className="max-w-3xl mx-auto py-8 px-4">
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[1,2,3].map(i => (
            <div key={i} className="rounded-xl p-4 text-center shadow-sm"
        ...
        {[1,2,3].map(i => (
          <div key={i} className="rounded-xl p-4 mb-3 border-l-4"
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: isDark ? '#0f1117' : '#f5f5f5' }}>
      ...
      <div className="max-w-3xl mx-auto py-8 px-4">
        ...
        {stats && (
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="rounded-xl p-4 text-center shadow-sm border-t-4 card-hover fade-in-delay-1"
              style={{ backgroundColor: isDark ? '#1a1d27' : 'white', borderTopColor: '#1e2d5a' }}>
              <div className="text-2xl font-bold" style={{ color: '#1e2d5a' }}>{stats.total_trips}</div>
              <div className="text-sm mt-1" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>{t('totalTrips')}</div>
            </div>
            <div className="rounded-xl p-4 text-center shadow-sm border-t-4 card-hover fade-in-delay-2"
              style={{ backgroundColor: isDark ? '#1a1d27' : 'white', borderTopColor: '#2d7a4f' }}>
              <div className="text-2xl font-bold text-green-600">{stats.confirmed_trips}</div>
              <div className="text-sm mt-1" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>{t('confirmed')}</div>
            </div>
            <div className="rounded-xl p-4 text-center shadow-sm border-t-4 card-hover fade-in-delay-3"
              style={{ backgroundColor: isDark ? '#1a1d27' : 'white', borderTopColor: '#8B1A2B' }}>
              <div className="text-2xl font-bold" style={{ color: '#8B1A2B' }}>{stats.total_bookings}</div>
              <div className="text-sm mt-1" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>{t('myBookings')}</div>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="text-white px-4 py-2 rounded-lg text-sm hover:opacity-90 transition btn-press"
            style={{ backgroundColor: '#8B1A2B' }}
          >
            {t('addTrip')}
          </button>
        ...
        {trips.map((trip, index) => (
          <div
            key={trip.id}
            className={`rounded-xl shadow-sm p-4 mb-3 border-l-4 card-hover fade-in-delay-${Math.min(index + 1, 5)}`}
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm mt-1" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>
                  Bus {trip.bus_number} · {new Date(trip.departure_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · {trip.booked_seats}/{trip.capacity} seats
                </p>
              </div>
                ...
                {trip.status !== 'cancelled' && trip.status !== 'completed' && (
                  <>
                    <button
                      ...
                      className="text-xs px-2 py-1 rounded border hover:opacity-80 transition btn-press"
                      style={{ color: '#1e2d5a', borderColor: '#1e2d5a' }}
                    >
                      {t('track')}
                    </button>
                    <button
                      onClick={() => navigate(`/add-trip?edit=${trip.id}&bus_id=${trip.bus_id}&area=${trip.area}&departure_time=${trip.departure_time}&direction=${trip.direction}`)}
                      className="text-xs px-2 py-1 rounded border hover:opacity-80 transition btn-press text-green-600"
                    ...
                    <button
                      ...
                      className="text-xs px-2 py-1 rounded border hover:opacity-80 transition btn-press"
                      style={{ color: '#8B1A2B', borderColor: '#8B1A2B' }}
                    >
                      {t('cancel')}
                    </button>
                  </>
        ...
        <div className="mt-8 mb-4">
          <button
            ...
            className="flex items-center gap-2 hover:opacity-80 transition btn-press"
            style={{ color: isDark ? '#ffffff' : '#1e2d5a' }}
          >
            <span className="text-lg font-semibold">{t('archivedTrips')}</span>
            <span className="text-sm px-2 py-0.5 rounded-full text-white"
              style={{ backgroundColor: '#1e2d5a' }}>
              {archivedTrips.length}
            </span>
            <span className="text-sm text-gray-400">
              {showArchive ? t('hide') : t('show')}
            </span>
          </button>
        </div>
        ...
        {showArchive && (
          archivedTrips.length === 0 ? (
            <div className="rounded-xl shadow-sm p-6 text-center mb-6 fade-in"
              style={{ backgroundColor: isDark ? '#1a1d27' : 'white' }}>
            ...
            archivedTrips.map((trip, index) => (
              <div
                key={trip.id}
                className={`rounded-xl p-4 mb-3 fade-in-delay-${Math.min(index + 1, 5)}`}
                style={{
                  backgroundColor: isDark ? '#1a1d27' : '#f9fafb',
                  border: `1px solid ${isDark ? '#2d3748' : '#e5e7eb'}`
                }}
              >
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="font-semibold" style={{ color: isDark ? '#a0aec0' : '#4b5563' }}>
                      {trip.direction === 'from_university'
                        ? `${t('israTo')} ${trip.area}`
                        : `${trip.area} ${t('toIsra')}`}
                    </h3>
                    <p className="text-sm mt-1" style={{ color: isDark ? '#718096' : '#9ca3af' }}>
                      Bus {trip.bus_number} · {new Date(trip.departure_time).toLocaleDateString()} · {trip.booked_seats}/{trip.capacity} seats
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleRestore(trip.id)}
                      className="text-xs px-2 py-1 rounded border text-green-600 hover:bg-green-50 transition btn-press"
                      style={{ borderColor: '#2d7a4f' }}
                    >
                      {t('restore')}
                    </button>
        ...
        <div className="mt-8">
          ...
          {reviews.length === 0 ? (
            <div className="rounded-xl shadow-sm p-6 text-center fade-in"
              style={{ backgroundColor: isDark ? '#1a1d27' : 'white' }}>
            ...
            reviews.map((review, index) => (
              <div
                key={review.id}
                className={`rounded-xl shadow-sm p-4 mb-3 border-l-4 card-hover fade-in-delay-${Math.min(index + 1, 5)}`}
                style={{ backgroundColor: isDark ? '#1a1d27' : 'white', borderLeftColor: '#8B1A2B' }}
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-medium text-sm" style={{ color: isDark ? '#ffffff' : '#1a202c' }}>
                      {review.student_name}
                    </p>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm font-medium ml-1" style={{ color: isDark ? '#a0aec0' : '#4b5563' }}>
                      {review.rating}/5
                    </span>
                  </div>
                </div>
