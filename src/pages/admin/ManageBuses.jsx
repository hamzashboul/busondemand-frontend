...
const ManageBuses = () => {
  ...
  const handleEdit = (bus) => {
    setEditingId(bus.id)
    setForm({ bus_number: bus.bus_number, capacity: bus.capacity, driver_name: bus.driver_name || '' })
    setMessage('')
  }
        ...
        {menuOpen && (
          <div className="md:hidden flex flex-col gap-2 pt-4 pb-2 fade-in"
            ...
            <button onClick={() => { navigate('/dashboard'); setMenuOpen(false) }}
              className="text-sm text-white border px-3 py-2 rounded-lg text-left"
              style={{ borderColor: 'rgba(255,255,255,0.4)' }}>
              {t('dashboard')}
            </button>
      ...
      <div className="max-w-2xl mx-auto py-8 px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            <div>
              <label className="block text-xs font-medium mb-1"
                style={{ color: isDark ? '#a0aec0' : '#1e2d5a' }}>{t('busNumber')}</label>
              <input type="text" name="bus_number" value={form.bus_number} onChange={handleChange}
                placeholder="e.g. B-03" required
                className="w-full border-2 rounded-lg px-3 py-2 text-sm focus:outline-none transition"
                style={{ borderColor: isDark ? '#2d3748' : '#e5e7eb', backgroundColor: isDark ? '#0f1117' : 'white', color: isDark ? '#ffffff' : '#1a202c' }}
                onFocus={(e) => e.target.style.borderColor = '#1e2d5a'}
                onBlur={(e) => e.target.style.borderColor = isDark ? '#2d3748' : '#e5e7eb'} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1"
                style={{ color: isDark ? '#a0aec0' : '#1e2d5a' }}>{t('capacity')}</label>
            ...
            <div>
              <label className="block text-xs font-medium mb-1"
                style={{ color: isDark ? '#a0aec0' : '#1e2d5a' }}>{t('driverName')}</label>
              <input type="text" name="driver_name" value={form.driver_name} onChange={handleChange}
          ...
          <div className="flex gap-3">
            ...
            {editingId && (
              <button type="button" onClick={handleCancelEdit}
                ...
                style={{ borderColor: '#1e2d5a', color: isDark ? '#ffffff' : '#1e2d5a', backgroundColor: 'transparent' }}>
                {t('cancel')}
              </button>
            )}
          </div>
        ...
        <div className="rounded-xl shadow-sm overflow-hidden"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {buses.map((bus) => (
                <tr key={bus.id} className="transition"
                  style={{
                    borderTop: `1px solid ${isDark ? '#2d3748' : '#f3f4f6'}`,
                    backgroundColor: editingId === bus.id ? (isDark ? '#1e2d5a22' : '#eff6ff') : 'transparent'
                  }}>
                  <td className="px-4 py-3 font-medium" style={{ color: isDark ? '#ffffff' : '#1a202c' }}>{bus.bus_number}</td>
                  <td className="px-4 py-3" style={{ color: isDark ? '#a0aec0' : '#4b5563' }}>{bus.capacity} seats</td>
                  <td className="px-4 py-3" style={{ color: isDark ? '#a0aec0' : '#4b5563' }}>{bus.driver_name || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${bus.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
