    ...
    ">
      <div style="background-color: #1e2d5a; padding: 20px; text-align: center;">
        <h2 style="color: white; margin: 0; font-size: 20px;">
          ${isArabic ? 'باص أون ديماند' : 'BusOnDemand'}
        </h2>
        <p style="color: #a8c4e0; margin: 4px 0 0; font-size: 12px;">
          ${isArabic ? 'جامعة الإسراء — نظام النقل' : 'Isra University — Transportation System'}
        </p>
      </div>
      ...
      <div style="padding: 24px; background: white;">
        <div style="text-align: center; margin-bottom: 20px;">
          <div style="background-color: #f0f3fa; border-radius: 8px; padding: 12px; display: inline-block;">
            <span style="font-size: 32px;">🚌</span>
          </div>
          <h3 style="color: #1e2d5a; margin: 8px 0 0; font-size: 16px;">
            ${isArabic ? 'تذكرة الحجز' : 'Booking Ticket'}
          </h3>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            ...
            <td style="color: #1e2d5a; font-weight: bold; font-size: 12px; text-align: ${isArabic ? 'left' : 'right'};">
              #${booking.id}
            </td>
          </tr>
          <tr>
            ...
            <td style="color: #1e2d5a; font-weight: bold; font-size: 12px; text-align: ${isArabic ? 'left' : 'right'};">
              ${booking.direction === 'from_university'
                ? isArabic
                  ? `جامعة الإسراء ← ${booking.area}`
                  : `Isra University → ${booking.area}`
                : isArabic
                  ? `${booking.area} ← جامعة الإسراء`
                  : `${booking.area} → Isra University`}
            </td>
          </tr>
          <tr>
            <td style="color: #6b7280; font-size: 12px; padding: 6px 0;">
          ...
          <tr>
            ...
            <td style="text-align: ${isArabic ? 'left' : 'right'};">
              <span style="
                background-color: ${booking.trip_status === 'confirmed' ? '#dcfce7' : '#fef9c3'};
                color: ${booking.trip_status === 'confirmed' ? '#166534' : '#854d0e'};
                padding: 2px 8px;
                border-radius: 20px;
                font-size: 11px;
                font-weight: bold;
              ">
                ${isArabic
                  ? booking.trip_status === 'confirmed' ? 'مؤكدة'
                  : booking.trip_status === 'pending' ? 'معلقة'
                  : booking.trip_status === 'cancelled' ? 'ملغاة' : 'مكتملة'
                  : booking.trip_status}
              </span>
            </td>
          </tr>
        ...
        <div style="text-align: center;">
          <p style="color: #9ca3af; font-size: 10px; margin: 0;">
            ${isArabic ? 'يرجى تقديم هذه التذكرة للسائق' : 'Please present this ticket to the driver'}
          </p>
          <p style="color: #9ca3af; font-size: 10px; margin: 4px 0 0;">
            📞 0798872267 | 0775443500
          </p>
        </div>
      </div>
      ...
      <div style="background-color: #8B1A2B; padding: 8px; text-align: center;">
        <p style="color: white; font-size: 10px; margin: 0;">
          ${isArabic ? 'جامعة الإسراء — نظام باص أون ديماند' : 'Isra University — BusOnDemand System'}
        </p>
      </div>
    </div>
  `
  ...
  📅 {t('schedule')}
      ...
      <div className="max-w-2xl mx-auto py-8 px-4">
        ...
        {bookings.length === 0 ? (
          <div className="text-center py-12 fade-in">
            ...
            <button
              onClick={() => navigate('/trips')}
              className="text-white px-6 py-2 rounded-lg text-sm hover:opacity-90 transition btn-press"
              style={{ backgroundColor: '#8B1A2B' }}
            >
              {t('browseTrips')}
            </button>
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="font-semibold"
                    ...
                    {booking.direction === 'from_university'
                      ? `${t('israTo')} ${booking.area}`
                      : `${booking.area} ${t('toIsra')}`}
                  </h3>
                ...
                <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                  booking.trip_status === 'confirmed' ? 'bg-green-100 text-green-700' :
                  booking.trip_status === 'cancelled' ? 'bg-red-100 text-red-700' :
              ...
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  className="flex-1 border py-2 rounded-lg text-sm font-medium disabled:opacity-50 hover:opacity-80 transition btn-press"
                  style={{ borderColor: '#8B1A2B', color: '#8B1A2B' }}
                >
                  {cancellingId === booking.id ? t('cancelling') : t('cancelBooking')}
                </button>
                <button
                  onClick={() => handlePrintTicket(booking)}
                  className="flex-1 border py-2 rounded-lg text-sm font-medium hover:opacity-80 transition btn-press"
                  style={{ borderColor: '#1e2d5a', color: isDark ? '#ffffff' : '#1e2d5a' }}
                >
                  {t('printTicket')}
                </button>
                <button
                  onClick={() => {
                    setReviewingId(reviewingId === booking.trip_id ? null : booking.trip_id)
                    setRating(0)
                    setComment('')
                  }}
                  className="flex-1 border py-2 rounded-lg text-sm font-medium hover:opacity-80 transition text-white btn-press"
              ...
              {reviewingId === booking.trip_id && (
                <div className="rounded-lg p-4 mt-2 fade-in"
                  style={{ backgroundColor: isDark ? '#0f1117' : '#f8f9ff' }}>
                  ...
                  <div className="flex gap-2 mb-3 justify-center">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        ...
                        className={`text-2xl transition-transform hover:scale-110 btn-press ${
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none mb-3 transition"
                    style={{
                    ...
                    onBlur={(e) => e.target.style.borderColor = isDark ? '#2d3748' : '#d1d5db'}
                  />

                  <button
                    onClick={() => handleReviewSubmit(booking.trip_id)}
                    disabled={reviewLoading || rating === 0}
                    className="w-full text-white py-2 rounded-lg text-sm font-medium disabled:opacity-50 hover:opacity-90 transition btn-press"
                    style={{ backgroundColor: '#8B1A2B' }}
                  >
      ...
      {confirmBookingId && (
        <ConfirmModal
          ...
          onConfirm={() => handleCancel(confirmBookingId)}
