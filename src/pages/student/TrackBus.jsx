...
const TrackBus = () => {
  ...
  const fetchTripInfo = async () => {
    try {
      ...
      setLocation({ lat: locationRes.data.latitude, lng: locationRes.data.longitude })
    } catch (error) {
      ...
      <div className="max-w-2xl mx-auto py-6 px-4">
        ...
        {tripInfo && (
          <div className="rounded-xl shadow-sm p-4 mb-4 border-l-4"
            ...
            <h3 className="font-semibold text-sm mb-1" style={{ color: isDark ? '#ffffff' : '#1a202c' }}>
              {tripInfo.direction === 'from_university' ? `${t('israTo')} ${tripInfo.area}` : `${tripInfo.area} ${t('toIsra')}`}
            </h3>
            <p className="text-xs" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>
        ...
        <div className="rounded-xl shadow-sm overflow-hidden mb-4 border-t-4"
          ...
          {location ? (
            ...
            <div className="h-48 flex items-center justify-center" style={{ backgroundColor: isDark ? '#0f1117' : '#f9fafb' }}>
              <p className="text-sm" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>{t('waitingLocation')}</p>
            </div>
          )}
        </div>

        {location && (
          <div className="rounded-xl shadow-sm p-4" style={{ backgroundColor: isDark ? '#1a1d27' : 'white' }}>
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-xs mb-1" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>{t('latitude')}</div>
                <div className="font-semibold text-sm" style={{ color: '#1e2d5a' }}>{location.lat.toFixed(5)}</div>
              </div>
              <div>
                <div className="text-xs mb-1" style={{ color: isDark ? '#a0aec0' : '#6b7280' }}>{t('longitude')}</div>
                <div className="font-semibold text-sm" style={{ color: '#1e2d5a' }}>{location.lng.toFixed(5)}</div>
              </div>
            </div>
          </div>
        )}
